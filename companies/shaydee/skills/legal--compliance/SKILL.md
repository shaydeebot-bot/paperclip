...
## Distillation Rules (from eval critiques)

These rules encode patterns that consistently improved legal compliance output across multiple evaluation iterations. Treat them as hard requirements.

### Data Mapping
1. **Map every data field individually with its lawful basis.** Never summarize with "we collect personal data." Create a field-by-field table mapping each data point to its source, purpose, GDPR lawful basis, and third-party sharing status. This is what compliance auditors expect.

### Health and Biometric Data
2. **Always address state-level biometric laws when health or biometric data is involved.** If the product collects heart rate, fingerprint, facial geometry, or similar data, include a dedicated section covering:
   - Illinois BIPA (written release requirement, cite section 15(e), retention and destruction schedule, 3-year inactivity trigger)
   - Texas CUBI
   - Washington RCW 19.375
   - Identify the specific sensor or collection method (e.g., "optical PPG sensor" for heart rate)
   - Provide a dedicated in-app path for biometric data rights
3. **Classify health/fitness data as GDPR Article 9 special category data** and require explicit consent. Do not treat health data as ordinary personal data.

### Breach Notification
4. **Specify breach notification timelines by jurisdiction.** Never use vague language like "notify you in accordance with applicable laws." Instead:
   - GDPR: 72-hour notification to supervisory authority (Art. 33), "without undue delay" to individuals when high risk (Art. 34)
   - CCPA: notification requirements with specific triggers
   - Include product-specific examples of what constitutes a reportable breach (e.g., unauthorized access to health data, credential compromise, GPS data exposure, ransomware, accidental vulnerability exposure)

### CCPA: Authorized Agent Process
5. **Detail the authorized agent verification process concretely.** Never leave it at "we will require verification." Specify:
   - Acceptable forms of agent authorization (signed permission letter with required fields, power of attorney under CA Probate Code sections 4000-4465, CA Secretary of State registration)
   - How the consumer's identity is verified when an agent makes the request (direct email to consumer)
   - Agent identity verification (government-issued ID)
   - Response timelines for agent-submitted requests

### Document Usability
6. **Use illustrative values with footnotes instead of raw placeholders.** Replace `[PLACEHOLDER]` tags with realistic example values (e.g., "HealthTrack, Inc., 123 Fitness Ave, San Francisco, CA 94102") marked with a dagger symbol and a footnote explaining these are illustrative and need customization. The main document should read as complete.
7. **Add plain-language summaries before caps-lock legal blocks.** Before any ALL-CAPS disclaimer or limitation of liability section, include an italicized plain-English summary: *"In plain terms: we provide the app as-is and can't guarantee it will always work perfectly."*
8. **Include a clickable table of contents for multi-document legal page sets.** Any output with more than one document (privacy policy + terms + cookie policy) must open with a TOC linking to every section and subsection.

### Cookie and Transfer Specificity
9. **List individual cookies by name.** Include provider, purpose, and duration for each cookie. Never summarize with "we use cookies for analytics."
10. **Specify international transfer mechanisms precisely.** Reference the June 2021 SCC version, correct module (C2P, C2C, P2P, P2C), UK addendum, transfer impact assessments, and supplementary measures.
